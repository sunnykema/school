...
public class KGFMapSystem : KGFModule, KGFICustomGUI, KGFIValidator, KGFICustomInspectorGUI
{
   ...
   // private Camera itsCamera = null;
    public static  Camera itsCamera = null;
    /// <summary>
    ...
    public void ShowPoints()
    {
        ...
        //Thread.Sleep(3000);
        //print("It has Sleeping!!!");
        print("Showpoints has been invoked");
        if (SerNearly.cnt == 0)
        {
            print("cnt is 0");
        }

        for (int i = 0; i < SerNearly.cnt; i++)
        {
            x = float.Parse(SerNearly.Xaxis[i]);        //获得数据库中的数据
            y = float.Parse(SerNearly.Yaxis[i]);
            z = float.Parse(SerNearly.Zaxis[i]);
           /* ChangePosition changeposition = new ChangePosition();
            //ChangePosition changepositiont = new ChangePosition();
            changeposition.ChangePeople(x, y, z);           //坐标转换
            po.x = changeposition.GetX();
    ...
    void AutoCreatePhoto()
    {
        ...
        if (itsDataModuleMinimap.itsPhoto.itsTakePhoto)
        {
            ...
            // show photo in plane under terrain
            {
                ...
                aPhotoPlanePosition.x -= 58.68707f;
                aPhotoPlanePosition.y = itsFixedYPhoto;
                aPhotoPlanePosition.z -= 80.11192f;
                aPhotoPlane.transform.position = aPhotoPlanePosition;
                aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 248.76343f, 1, itsTerrainBounds.size.z - 131.4921f);
                /*
        ...
        {               //如果满足条件，则更换掉小地图的图片
            ...
            if (itsDataModuleMinimap.itsPhoto.itsTakePhoto)
            {
                ...
                // show photo in plane under terrain
                {
                    ...
                    {
                        flag2 = false;
                        aPhotoPlanePosition.x -= 123.47f;
                        aPhotoPlanePosition.z -= 69.72f;
                        aPhotoPlane.transform.position = aPhotoPlanePosition;
                        aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 167.79f, 1, itsTerrainBounds.size.z - 148.42f);//Vector3.one * Mathf.Max(aBounds.size.x,aBounds.size.z);
                    }
                    else if (flag3)
                    {
                        flag3 = false;
                        ...
                        aPhotoPlanePosition.z -= 69.72f;
                        aPhotoPlane.transform.position = aPhotoPlanePosition;
                        aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 167.79f, 1, itsTerrainBounds.size.z - 148.42f);
                    }
                    //修改小地图的大小
                }
            }
    ...
    /*public static void UpdateOrthographicSize1()
    {
        itsCamera.orthographicSize += 10.0f;
        //itsCamera.aspect = ((float)GetWidth()) / ((float)GetHeight());
        //		itsCamera.ResetAspect();
    }
    public static void UpdateOrthographicSize2()
    {
        itsCamera.orthographicSize -= 10.0f;
        //itsCamera.aspect = ((float)GetWidth()) / ((float)GetHeight());
        //		itsCamera.ResetAspect();
